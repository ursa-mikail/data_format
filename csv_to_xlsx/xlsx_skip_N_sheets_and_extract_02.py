...
def extract_ip_data(excel_file_path, skip_sheets=4, start_row=7, output_file=None):
    ...
    try:
        ...
        if all_ip_data:
            ...
            print(f"\n🎉 Total records extracted: {len(all_ip_data)}")
            return result_df
        else:
...
📄 Processing sheet: use1
   Full sheet dimensions: 10 rows x 6 columns
...
Rows 6-15 in use1:
  Row 6: ['Virtual machine name', 'IP', 'Cores', 'GRAM', 'GDISK', 'Username/password']
  Row 7: ['', '10.0.0.0', 'ID', '', '', '']
  Row 8: ['', '10.0.0.1', 'DEFAULT GATEWAY', '', '', '']
  Row 9: ['test-server1', '10.0.0.26', '', '', '', 'test/test']
  Row 10: ['test-server2', '10.0.0.30', '', '', '', 'test/test']
...
Rows 6-15 in use2:
  ...
  Row 9: ['visu1', '10.0.1.2', '', '', '', 'test/test']
  Row 10: ['gpu1', '10.0.1.3', '', '', '', 'test/test']
"""  